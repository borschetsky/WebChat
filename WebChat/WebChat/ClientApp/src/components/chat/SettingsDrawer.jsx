...
export default function SettingsDrawer({
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose} slotProps={{ paper: { sx: { width: fullWidth ? '100%' : 360 } } }}>
      <Stack direction="row" alignItems="center" gap={1.5} sx={{ p: 1.75, borderBottom: 1, borderColor: 'divider' }}>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 2, py: 2.5 }}>
        <Stack alignItems="center" gap={1} sx={{ pb: 2.5 }}>
        ...
        {members?.length > 0 && (
          <>
            ...
            {members.map((p) => (
              <Stack key={p.id ?? p.name} direction="row" alignItems="center" gap={1.5} sx={{ px: 1, py: 1.1 }}>
