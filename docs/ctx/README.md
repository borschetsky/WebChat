...
|---|---|---|---|
| 2026-08-03 | [Redux Toolkit + RTK Query refactor of the client](2026-08-03-redux-toolkit-refactor.md) | change | Slices for auth/ui/composer and RTK Query over `chat-service` so the mock seam survives. `ChatApp` 19 → 7 `useState`; the composer draft leaves the shared render path. Optimistic send with a retry affordance. Forced the service layer to TypeScript. |
